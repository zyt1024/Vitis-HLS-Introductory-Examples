...
//------------------------Parameter----------------------
`define TV_IN_outTop "../tv/cdatafile/c.example.autotvin_outTop.dat"
parameter ADDR_WIDTH = 5;
parameter DATA_WIDTH = 32;
parameter outTop_DEPTH = 1;
reg [31 : 0] outTop_OPERATE_DEPTH = 0;
parameter outTop_c_bitwidth = 64;
parameter START_ADDR = 0;
parameter example_continue_addr = 0;
parameter example_auto_start_addr = 0;
parameter outTop_data_in_addr = 16;
...
reg  BREADY_reg = 0;
reg [outTop_c_bitwidth - 1 : 0] mem_outTop [outTop_DEPTH - 1 : 0] = '{default : 'h0};
reg [DATA_WIDTH-1 : 0] image_mem_outTop [ (outTop_c_bitwidth+DATA_WIDTH-1)/DATA_WIDTH * outTop_DEPTH -1 : 0] = '{default : 'hz};
reg outTop_write_data_finish;
...
//write outTop reg
reg [31 : 0] write_outTop_count = 0;
reg write_outTop_run_flag = 0;
reg write_one_outTop_data_done = 0;
...
assign TRAN_control_write_data_finish = 1 & outTop_write_data_finish;
...
always @(reset or posedge clk) begin
    if (reset == 0) begin
        outTop_write_data_finish <= 0;
        write_outTop_run_flag <= 0; 
        write_outTop_count = 0;
        count_operate_depth_by_bitwidth_and_depth (outTop_c_bitwidth, outTop_DEPTH, outTop_OPERATE_DEPTH);
    end
    else begin
        ...
        if (AESL_ready_reg === 1) begin
            write_outTop_run_flag <= 1; 
            write_outTop_count = 0;
        end
        if (write_one_outTop_data_done === 1) begin
            write_outTop_count = write_outTop_count + 1;
            if (write_outTop_count == outTop_OPERATE_DEPTH) begin
                write_outTop_run_flag <= 0; 
                outTop_write_data_finish <= 1;
            end
        end
    end
end

initial begin : write_outTop
    integer write_outTop_resp;
    integer process_num ;
    integer get_ack;
    ...
    integer i;
    integer j;
    reg [31 : 0] outTop_data_tmp_reg;
    wait(reset === 1);
    @(posedge clk);
    c_bitwidth = outTop_c_bitwidth;
    process_num = 1;
    ...
    while (1) begin
        ...
        if (ongoing_process_number === process_num && process_busy === 0 ) begin
            get_ack = 1;
            if (write_outTop_run_flag === 1 && get_ack === 1) begin
                process_busy = 1;
                //write outTop data 
                for (i = 0 ; i < four_byte_num ; i = i+1) begin
                    if (outTop_c_bitwidth < 32) begin
                        outTop_data_tmp_reg = mem_outTop[write_outTop_count];
                    end
                    else begin
                        for (j=0 ; j<32 ; j = j + 1) begin
                            if (i*32 + j < outTop_c_bitwidth) begin
                                outTop_data_tmp_reg[j] = mem_outTop[write_outTop_count][i*32 + j];
                            end
                            else begin
                                outTop_data_tmp_reg[j] = 0;
                            end
                        end
                    end
                    if(image_mem_outTop[write_outTop_count * four_byte_num  + i]!==outTop_data_tmp_reg) begin
                    write (outTop_data_in_addr + write_outTop_count * four_byte_num * 4 + i * 4, outTop_data_tmp_reg, write_outTop_resp);
                    image_mem_outTop[write_outTop_count * four_byte_num + i]=outTop_data_tmp_reg;
                    end
                end
                process_busy = 0;
                write_one_outTop_data_done <= 1;
                @(posedge clk);
                write_one_outTop_data_done <= 0;
            end   
            process_1_finish <= 1;
        end
        @(posedge clk);
...
// Read data from file 
initial begin : read_outTop_file_process 
  integer fp; 
  integer ret; 
  integer factor; 
  reg [151 : 0] token; 
  reg [151 : 0] token_tmp; 
  //reg [outTop_c_bitwidth - 1 : 0] token_tmp; 
  ...
  count_seperate_factor_by_bitwidth (outTop_c_bitwidth , factor);
  fp = $fopen(`TV_IN_outTop ,"r"); 
  if(fp == 0) begin                               // Failed to open file 
      $display("Failed to open file \"%s\"!", `TV_IN_outTop); 
      $finish; 
  end 
  read_token(fp, token); 
  ...
  while (token != "[[[/runtime]]]") begin 
      ...
      for(i = 0; i < outTop_DEPTH; i = i + 1) begin 
          read_token(fp, token); 
          ...
          if (factor == 4) begin
              ...
              if (i%factor == 3) begin
                  ...
                  mem_outTop [i/factor] = tmp_cache_mem;
                  tmp_cache_mem [DATA_WIDTH - 1 : 0] = 0;
              end
          end
          if (factor == 2) begin
